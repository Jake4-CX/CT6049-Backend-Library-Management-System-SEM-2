...
@Path("/librarian/popular/currently-loaned/category")
public class MostPopularBookCategoryCurrentlyLoanedResource {

    @GET
    @RestrictedRoles({User.Role.LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getMostPopularBookCategoryCurrentlyLoaned() {
        LibrarianService service = new LibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            String result = service.getMostPopularBookCategoryCurrentlyLoaned();
