...
@Path("/chief-librarian/average/books-author")
public class AverageBooksPerAuthorResource {

    @GET
    @RestrictedRoles({User.Role.CHIEF_LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAverageBooksPerAuthor() {
        ChiefLibrarianService service = new ChiefLibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            double result = service.getAverageBooksPerAuthor();
