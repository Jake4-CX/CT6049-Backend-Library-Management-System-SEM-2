...
@Path("/librarian/books-per-category")
public class BooksPerCategoryResource {

    @GET
    @RestrictedRoles({User.Role.LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getBooksPerCategory() {
        LibrarianService service = new LibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            HashMap<String, Object>[] result = service.getBooksPerCategory();
