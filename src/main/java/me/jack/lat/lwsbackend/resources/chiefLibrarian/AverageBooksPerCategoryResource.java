...
@Path("/chief-librarian/average/books-category")
public class AverageBooksPerCategoryResource {

    @GET
    @RestrictedRoles({User.Role.CHIEF_LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAverageBooksPerCategory() {
        ChiefLibrarianService service = new ChiefLibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            double result = service.getAverageBooksPerCategory();
