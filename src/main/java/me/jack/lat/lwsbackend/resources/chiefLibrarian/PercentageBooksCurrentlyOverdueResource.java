...
@Path("/chief-librarian/percentage/books-overdue")
public class PercentageBooksCurrentlyOverdueResource {

    @GET
    @RestrictedRoles({User.Role.CHIEF_LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPercentageBooksCurrentlyOverdue() {
        ChiefLibrarianService service = new ChiefLibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            double result = service.getPercentageBooksCurrentlyOverdue();
