...
@Path("/finance-director/popular/unpaid/category")
public class MostPopularCategoryUnpaidFinesResource {

    @GET
    @RestrictedRoles({User.Role.FINANCE_DIRECTOR})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getMostPopularCategoryUnpaidFines() {
        FinanceDirectorService service = new FinanceDirectorService();
        Map<String, Object> response = new HashMap<>();

        try {
            String result = service.getMostPopularCategoryUnpaidFines();
