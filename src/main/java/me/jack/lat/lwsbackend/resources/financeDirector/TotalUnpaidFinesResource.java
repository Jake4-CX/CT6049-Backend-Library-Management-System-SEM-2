...
@Path("/finance-director/total/unpaid-fines")
public class TotalUnpaidFinesResource {

    @GET
    @RestrictedRoles({User.Role.FINANCE_DIRECTOR})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getTotalUnpaidFines() {
        FinanceDirectorService service = new FinanceDirectorService();
        Map<String, Object> response = new HashMap<>();

        try {
            double result = service.getTotalUnpaidFines();
