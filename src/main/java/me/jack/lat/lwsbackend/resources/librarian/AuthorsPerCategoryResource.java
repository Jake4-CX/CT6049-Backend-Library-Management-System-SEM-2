...
@Path("/librarian/authors-per-category")
public class AuthorsPerCategoryResource {

    @GET
    @RestrictedRoles({User.Role.LIBRARIAN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAuthorsPerCategory() {
        LibrarianService service = new LibrarianService();
        Map<String, Object> response = new HashMap<>();

        try {
            HashMap<String, Object>[] result = service.getAuthorsPerCategory();
