...
@Path("/finance-director/percentage/unpaid-fines")
public class PercentageUnpaidFinesResource {

    @GET
    @RestrictedRoles({User.Role.FINANCE_DIRECTOR})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPercentageUnpaidFines() {
        FinanceDirectorService service = new FinanceDirectorService();
        Map<String, Object> response = new HashMap<>();

        try {
            double result = service.getPercentageUnpaidFines();
